...
function Firmware_Update(NodeID, FileName, CheckMaster, CheckDate)
{
	...
	if (CheckMaster && (BoardBranch !== "master"))
	{
	...
	if (CheckDate && (BoardDate >= FileDate))
	{
...
function Firmware_WaitInit()
{
	var sleep_time = 10
	var timeout = 5000 / sleep_time
	var counter = 0
	while (counter < timeout)
	{
		try
		{
			// 23 - последний доступный регистр в загрузчике
			dev.Read16Silent(24)
			return
		}
		catch (e)
